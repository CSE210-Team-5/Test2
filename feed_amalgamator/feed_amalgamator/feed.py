    ...
    if request.method == 'GET':
        ...
        if error is None:
            ...
            for user_server in user_servers:
                ...
                bot_m = Mastodon(client_id=CLIENT_ID, client_secret=CLIENT_SECRET,
                                 access_token=ACCESS_TOKEN, api_base_url=domain)
                print(domain, token)
                users_access_token = bot_m.log_in(code=token,
                                                  redirect_uri='urn:ietf:wg:oauth:2.0:oob',
                                                  scopes=["read", "write", "push"])
                user_client = Mastodon(access_token=users_access_token, api_base_url=domain)
